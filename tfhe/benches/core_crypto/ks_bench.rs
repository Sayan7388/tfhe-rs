...
fn benchmark_parameters_64bits() -> Vec<(String, CryptoParametersRecord<u64>)> {
    let classic = SHORTINT_BENCH_PARAMS
        ...
        .map(|params| {
            (
                params.name(),
                <ClassicPBSParameters as Into<PBSParameters>>::into(params.clone())
                    .to_owned()
                    .into(),
            )
        })
    ...
    let multi_bit = SHORTINT_MULTI_BIT_BENCH_PARAMS
        ...
        .map(|params| {
            (
                params.name(),
                <MultiBitPBSParameters as Into<PBSParameters>>::into(params.clone())
                    .to_owned()
                    .into(),
            )
        })
...
mod cuda {
    use crate::utilities::{write_to_json, CryptoParametersRecord, OperatorType};
    use crate::{benchmark_parameters_32bits, benchmark_parameters_64bits, keyswitch};
