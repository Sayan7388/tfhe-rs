...
use crate::integer::gpu::ciphertext::{CudaIntegerRadixCiphertext, CudaSignedRadixCiphertext, CudaUnsignedRadixCiphertext};
use crate::integer::gpu::CudaServerKey;
use crate::integer::server_key::radix_parallel::scalar_div_mod::{choose_multiplier, SignedReciprocable};
...
impl CudaServerKey {
            ...
            <<Scalar as SignedReciprocable>::Unsigned as Reciprocable>::DoublePrecision: Send,
    {
        ...
        let numerator_bits = self.message_modulus.0.ilog2() * numerator.ciphertext.d_blocks.lwe_ciphertext_count().0 as u32;
        assert!(
            Scalar::BITS >= numerator_bits as usize,
            "The scalar divisor type must have a number of bits that is\
            >= to the number of bits encrypted in the ciphertext"
        ...
        if chosen_multiplier.l >= numerator_bits {
            return self.create_trivial_zero_radix(numerator.ciphertext.d_blocks.lwe_ciphertext_count().0, streams);
        }
            ...
            < (<Scalar::Unsigned as Reciprocable>::DoublePrecision::ONE << (numerator_bits - 1))
        {
            ...
            let (mut tmp, xsign) = rayon::join(
                move || {
                    // MULSH(m, n)
                    let mut tmp =
                        self.signed_scalar_mul_high(numerator, chosen_multiplier.multiplier, streams);
                ...
                || {
                    ...
                    self.unchecked_scalar_right_shift(
                        numerator,
                        numerator_bits - 1,
                        streams,
                    )
