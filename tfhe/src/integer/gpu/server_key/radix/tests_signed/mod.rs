...
impl<'a, F> FunctionExecutor<(&'a SignedRadixCiphertext, i64), (SignedRadixCiphertext, SignedRadixCiphertext)>
for GpuFunctionExecutor<F>
    where
        F: Fn(
            &CudaServerKey,
        ...
        ) -> (CudaSignedRadixCiphertext, CudaSignedRadixCiphertext),
{
    ...
    fn execute(&mut self, input: (&'a SignedRadixCiphertext, i64)) -> (SignedRadixCiphertext, SignedRadixCiphertext) {
        let context = self
            .context
            .as_ref()
            .expect("setup was not properly called");
        ...
        let (gpu_result_1, gpu_result_2) = (self.func)(&context.sks, &d_ctxt_1, input.1, &context.streams);

        (gpu_result_1.to_signed_radix_ciphertext(&context.streams), gpu_result_2.to_signed_radix_ciphertext(&context.streams))
    }
}
