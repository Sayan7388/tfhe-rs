...
use crate::integer::server_key::radix_parallel::tests_signed::{random_non_zero_signed_value_under_modulus, random_signed_value_under_modulus, signed_div_under_modulus, signed_rem_under_modulus};
...
{
    let executor = CpuFunctionExecutor::new(&ServerKey::unchecked_signed_scalar_div_rem_parallelized);
...
pub(crate) fn signed_unchecked_scalar_div_rem_test<P, T>(param: P,  mut executor: T)
    where
        P: Into<PBSParameters>,
        T: for<'a> FunctionExecutor<(&'a SignedRadixCiphertext, i64), (SignedRadixCiphertext, SignedRadixCiphertext)>,
{
    ...
    executor.setup(&cks, sks);
    {
        ...
        let result = std::panic::catch_unwind(|| {
            let _ = executor.execute((&ctxt_0, 0));
        });
