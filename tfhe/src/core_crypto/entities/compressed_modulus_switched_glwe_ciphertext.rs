...
impl<Scalar: UnsignedTorus> CompressedModulusSwitchedGlweCiphertext<Scalar> {
    ...
    pub fn compress<Cont: Container<Element = Scalar>>(
        ...
        assert!(
            bodies_count <= polynomial_size.0,
            "Number of stored bodies (asked {bodies_count}) cannot be bigger than polynomial_size ={}",
            polynomial_size.0,
        );
        ...
        let modulus_switched: Vec<_> = ct.as_ref()
            [0..glwe_dimension.0 * polynomial_size.0 + bodies_count]
            .iter()
    ...
    pub fn extract(&self) -> GlweCiphertextOwned<Scalar> {
        ...
        let number_bits_to_unpack =
            (self.glwe_dimension.0 * self.polynomial_size.0 + self.bodies_count) * log_modulus;
        ...
        let container = self
            ...
            .chain(std::iter::repeat(Scalar::ZERO).take(self.polynomial_size.0 - self.bodies_count))
            .collect();
...
{
    type ParameterSet = GlweCiphertextParameters<Scalar>;

    fn is_conformant(&self, lwe_ct_parameters: &GlweCiphertextParameters<Scalar>) -> bool {
        let log_modulus = self.packed_integers.log_modulus.0;
